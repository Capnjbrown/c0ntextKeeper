#!/usr/bin/env node
/**
 * Test script for PostToolUse hook
 * Simulates tool usage events that Claude Code would send
 */

const { spawn } = require('child_process');
const path = require('path');

// Test data for PostToolUse event
const testEvent = {
...
hookProcess.on('close', (code) => {
  ...
  if (stdout) {
    try {
      ...
      console.log("✅ Success:", response.message || "Hook executed successfully");
      if (response.stats) {
  ...
  if (code === 0) {
    ...
    console.log("\n💡 Check the patterns folder for captured data:");
    console.log("   ~/.c0ntextkeeper/archive/projects/*/patterns/");
    
    // If debug mode is enabled, mention the debug logs
    if (process.env.C0NTEXTKEEPER_DEBUG === 'true') {
      console.log("\n📝 Debug logs available at:");
      console.log("   ~/.c0ntextkeeper/debug/posttool-*.log");
    }
  } else {
