          ...
          if (this.config.formatStyle === 'summary') {
            ...
            if (problems.length > 0) {
              ...
              problems.forEach((p: any) => {
                const question = this.truncateText(p.question, 150);
                sections.push(`- ${question}`);
                if (p.solution) {
                  const solution = this.truncateText(p.solution.approach, 100);
            ...
            if (implementations.length > 0) {
              ...
              implementations.forEach((impl: any) => {
                const desc = this.truncateText(impl.description, 100);
