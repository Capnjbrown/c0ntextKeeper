...
import { ExtractedContext, Problem, Implementation, Decision } from '../core/types.js';
...
function getDescriptionFromProblems(problems: Problem[]): string {
  ...
  if (keywords.includes('error') || keywords.includes('bug') || keywords.includes('fix')) {
    const errorType = findErrorType(problems);
    return errorType ? `fix-${errorType}` : 'bug-fix';
  }
  
  if (keywords.includes('implement') || keywords.includes('create') || keywords.includes('add')) {
  ...
  const significant = keywords.find(k => 
    !['the', 'and', 'or', 'but', 'how', 'what', 'when', 'where', 'why'].includes(k.toLowerCase())
  );
 ...
 */
function getDescriptionFromImplementations(implementations: Implementation[]): string {
  // Look for common implementation patterns
  const allCode = implementations.map(i => i.description).join(' ');
  
  if (allCode.includes('auth') || allCode.includes('jwt') || allCode.includes('login')) {
    return 'authentication-setup';
  }
  
  if (allCode.includes('test') || allCode.includes('describe') || allCode.includes('expect')) {
    return 'test-implementation';
  }
  
  if (allCode.includes('api') || allCode.includes('endpoint') || allCode.includes('route')) {
    return 'api-development';
  }
  
  if (allCode.includes('database') || allCode.includes('query') || allCode.includes('schema')) {
    return 'database-work';
  }
  
  // Look at file paths
  const paths = implementations.map(i => i.file);
  if (paths.some(p => p.includes('component'))) return 'component-development';
  if (paths.some(p => p.includes('style') || p.includes('css'))) return 'styling-updates';
...
function getDescriptionFromDecisions(decisions: Decision[]): string {
  const allDecisions = decisions.map(d => d.decision).join(' ').toLowerCase();
  ...
  if (allDecisions.includes('library') || allDecisions.includes('package')) return 'dependency-choices';
...
export function extractProjectName(projectPath: string): string {
  ...
  const genericNames = ['project', 'projects', 'validation', 'test', 'tests', 'temp', 'tmp', 'work', 'workspace'];
