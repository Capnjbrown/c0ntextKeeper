...
export class HooksManager {
  ...
  async enableHook(hookName: string): Promise<void> {
    ...
    if (!HOOK_DEFINITIONS[normalizedName]) {
      ...
      console.log('\nAvailable hooks:', Object.keys(HOOK_DEFINITIONS).join(', '));
      return;
    }
    
    const settings = this.loadSettings();
    const hookDef = HOOK_DEFINITIONS[normalizedName];
    const hookPath = path.join(this.projectRoot, 'dist', 'hooks', hookDef.script);
  ...
  async testHook(hookName: string): Promise<void> {
    ...
    try {
      const result = execSync(`echo '${JSON.stringify(testInput)}' | node ${hookPath}`, {
        encoding: 'utf-8',
    ...
    switch (hookName) {
      case 'PreCompact':
        return {
          ...
          transcript_path: path.join(this.projectRoot, 'tests', 'fixtures', 'sample-transcript.jsonl'),
          trigger: 'manual',
