...
export async function initCommand(options: InitOptions = {}): Promise<void> {
  ...
  try {
    if (globalInit) {
      ...
      logger.info(
        `Initializing global c0ntextkeeper storage at: ${globalPath}`,
      );
...
`;
        ...
        logger.info(
          "ℹ No .gitignore found - consider adding .c0ntextkeeper/ to version control exclusions",
        );
      }
    }
...
export async function statusCommand(): Promise<void> {
  ...
  if (info.exists) {
    ...
    logger.info(
      `  Location: ${info.type === "local" ? info.local : info.global}`,
    );
  ...
  if (fs.existsSync(globalPath)) {
    ...
    if (fs.existsSync(globalIndexPath)) {
      ...
      const validProjects = Object.entries(projects).filter(
        ([_, projectInfo]: [string, any]) => {
