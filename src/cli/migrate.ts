...
export class ArchiveMigrator {
  ...
  constructor(basePath?: string) {
    this.basePath =
      basePath ||
      path.join(process.env.HOME || "", ".c0ntextkeeper", "archive");
  ...
  async migrate(dryRun = false): Promise<{
    ...
    try {
          ...
          if (!dryRun) {
            ...
            if (await fileExists(sessionsDir)) {
              ...
              for (const sessionFile of sessions) {
                ...
                this.logger.info(
                  `Migrated session: ${sessionFile} → ${newFileName}`,
                );
              }
            }
            ...
            await this.createProjectReadme(
              newPath,
              projectName,
              index.projectPath,
            );
      ...
      this.logger.info(
        `Migration complete: ${result.migrated} projects migrated`,
      );
