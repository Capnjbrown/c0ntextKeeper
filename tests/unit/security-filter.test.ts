...
describe("SecurityFilter", () => {
  ...
  describe("filterText", () => {
    ...
    it("should redact Anthropic API keys", () => {
      const text =
        "export ANTHROPIC_API_KEY=sk-ant-api03-1234567890abcdefghijklmnopqrstuvwxyz1234567890";
      const filtered = filter.filterText(text);
      expect(filtered).toContain("[REDACTED]");
...
-----END RSA PRIVATE KEY-----`;
  ...
  describe("filterObject", () => {
    ...
    it("should handle arrays in objects", () => {
      const obj = {
        logs: ["API_KEY=secret", "Normal log entry", "password: test123"],
      };
  ...
  describe("containsSensitiveData", () => {
    ...
    it("should not flag normal text", () => {
      ...
      expect(filter.containsSensitiveData("function calculate() {}")).toBe(
        false,
      );
    });
  });
