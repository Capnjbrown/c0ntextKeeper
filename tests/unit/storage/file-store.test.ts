...
} = require("../../../src/utils/path-resolver");
const {
  generateSessionName,
  extractProjectName,
} = require("../../../src/utils/session-namer");
const {
  formatTimestamp,
  formatRelevance,
  truncateText,
...
describe("FileStore", () => {
  ...
  beforeEach(() => {
    ...
    generateSessionName.mockReturnValue(
      "2024-09-04_1000_MT_authentication-fix.json",
    );
    formatTimestamp.mockReturnValue("September 4, 2024 10:00 AM MT");
    formatRelevance.mockReturnValue("85%");
    truncateText.mockImplementation((text: string, len: number) =>
      text.slice(0, len),
    );
  ...
  describe("getBySessionId", () => {
    ...
    it("should throw on file read errors (no try-catch in implementation)", async () => {
      ...
      fileExists
        .mockResolvedValueOnce(true) // Projects dir exists
      ...
      await expect(fileStore.getBySessionId("test-session")).rejects.toThrow(
        "File corrupted",
      );
    });
  });
  ...
  describe("Session Cleanup", () => {
    ...
    it("should report but not delete when dryRun is set", async () => {
      ...
      const pruned = await shortRetentionStore.pruneOldSessions(
        "/tmp/project",
        {
          dryRun: true,
        },
  ...
  describe("Error Handling", () => {
    it("should handle permission denied errors", async () => {
      ...
      await expect(fileStore.store(mockContext)).rejects.toThrow(
        "Permission denied",
      );
    });
  ...
  describe("Index Management", () => {
    ...
    it("should update global index on store", async () => {
      ...
      const globalIndexCall = mockedFs.writeFile.mock.calls.find(
        (call) =>
          typeof call[0] === "string" &&
          call[0].includes("global") &&
          call[0].includes("index.json"),
