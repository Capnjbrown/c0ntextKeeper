...
describe("ContextRetriever", () => {
  ...
  describe("searchArchive", () => {
    it("should search with query", async () => {
      ...
      expect(mockFileStore.searchAll).toHaveBeenCalledWith(
        expect.any(Function),
      );
      expect(results).toBeDefined();
    });
    ...
    it("should sort by frequency when specified", async () => {
      ...
      if (results.length > 1) {
        for (let i = 0; i < results.length - 1; i++) {
          expect(results[i].matches.length).toBeGreaterThanOrEqual(
            results[i + 1].matches.length,
          );
        }
      }
    });
  ...
  describe("getProjectIndex", () => {
    it("should get project index", async () => {
      ...
      expect(mockFileStore.getProjectIndex).toHaveBeenCalledWith(
        "/test/project",
      );
  ...
  describe("getRecentContexts", () => {
    ...
    it("should handle storage errors gracefully", async () => {
      ...
      await expect(retriever.getRecentContexts(10)).rejects.toThrow(
        "Storage error",
      );
    });
  });
