...
describe("PatternAnalyzer", () => {
  ...
  describe("getPatterns", () => {
    it('should get all patterns when type is "all"', async () => {
      mockFileStore.searchAll = jest
        .fn()
        .mockResolvedValue([mockContext1, mockContext2]);
    ...
    it("should respect limit parameter", async () => {
      ...
      mockFileStore.searchAll = jest
        .fn()
        .mockResolvedValue([contextWithManyPatterns]);
    ...
    it("should filter by project path when provided", async () => {
      mockFileStore.getProjectContexts = jest
        .fn()
        .mockResolvedValue([mockContext1]);
    ...
    it("should aggregate patterns across contexts", async () => {
      ...
      mockFileStore.searchAll = jest
        .fn()
        .mockResolvedValue([mockContext1, context3]);
    ...
    it("should handle contexts without patterns", async () => {
      ...
      mockFileStore.searchAll = jest
        .fn()
        .mockResolvedValue([contextWithoutPatterns]);
    ...
    it("should sort patterns by frequency descending", async () => {
      ...
      mockFileStore.searchAll = jest
        .fn()
        .mockResolvedValue([contextWithPatterns]);
    ...
    it("should handle storage errors gracefully", async () => {
      ...
      await expect(analyzer.getPatterns(input)).rejects.toThrow(
        "Storage error",
      );
    });
  });
  ...
  describe("pattern merging", () => {
    it("should merge patterns with same type and value", async () => {
      ...
      mockFileStore.searchAll = jest
        .fn()
        .mockResolvedValue([context1, context2]);
