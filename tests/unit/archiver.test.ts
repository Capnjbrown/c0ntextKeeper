  ...
  describe("archiveFromTranscript", () => {
    it("should process a JSONL transcript and archive it", async () => {
      ...
      const parseTranscript =
        require("../../src/utils/transcript").parseTranscript;
      ...
      // Mock storage
      mockFileStore.store = jest
        .fn()
        .mockResolvedValue("/mock/archive/path.json");

      const result = await archiver.archiveFromTranscript(
        transcriptPath,
        "/test/project",
      );
    ...
    it("should always store context even with minimal content", async () => {
      ...
      mockFileStore.store = jest
        .fn()
        .mockResolvedValue("/mock/archive/minimal.json");
    ...
    it("should handle timeout gracefully", async () => {
      ...
      const parseTranscript =
        require("../../src/utils/transcript").parseTranscript;
      parseTranscript.mockRejectedValue(
        new Error("Parsing timed out after 40000ms"),
      );
    ...
    it("should log extraction details", async () => {
      ...
      mockFileStore.store = jest
        .fn()
        .mockResolvedValue("/mock/archive/detailed.json");
