...
jest.mock("../../../src/utils/hook-storage", () => ({
  ...
  getHookStorageDir: jest.fn(
    () => "/mock/.c0ntextkeeper/archive/projects/test-project/patterns",
  ),
}));

// Mock test-helpers
jest.mock("../../../src/utils/test-helpers", () => ({
  isTestSession: jest.fn(
    (sessionId: string) =>
...
describe("PostToolUse Hook", () => {
  beforeEach(() => {
    ...
    mockWriteHookData.mockReturnValue(
      "/mock/storage/path/patterns/2024-01-15_1030_MT_ion-123-patterns.json",
    );
  ...
  describe("extractToolPattern", () => {
    describe("standard tools", () => {
      ...
      it("should extract pattern for WebSearch tool", () => {
        ...
        const pattern = extractToolPattern(input);
        expect(pattern).toBe(
          'WebSearch: "typescript best practices" - success',
        );
      });
    ...
    describe("MCP tools", () => {
      it("should extract pattern for filesystem MCP tool", () => {
        ...
        const pattern = extractToolPattern(input);
        expect(pattern).toBe(
          "MCP filesystem: write file on /src/index.ts - success",
        );
      });
      ...
      it("should extract pattern for sequential-thinking MCP tool", () => {
        ...
        const pattern = extractToolPattern(input);
        expect(pattern).toBe(
          "MCP sequential-thinking: sequentialthinking - thought 3 of 5",
        );
      });
  ...
  describe("trackErrorPattern", () => {
    ...
    it("should handle corrupted error file gracefully", async () => {
      ...
      // Should not throw
      await expect(
        trackErrorPattern(pattern, mockStorage),
