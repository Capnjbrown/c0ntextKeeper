...
describe("PreCompact Hook Handler", () => {
  ...
  beforeEach(() => {
    ...
    (
      ContextArchiver as jest.MockedClass<typeof ContextArchiver>
    ).mockImplementation(() => mockArchiver);
  ...
  describe("valid PreCompact hook input", () => {
    it("should process valid PreCompact hook input successfully", async () => {
      ...
      mockArchiver.archiveFromTranscript = jest
        .fn()
        .mockResolvedValue(successArchiveResult);
    ...
    it("should error when transcript path is not provided", async () => {
      ...
      const errorOutput = calls.find((call) => {
        try {
          ...
          return (
            parsed.status === "error" &&
            parsed.message?.includes("No transcript path")
          );
        } catch {
          return false;
        }
    ...
    it("should error when transcript file does not exist", async () => {
      ...
      const errorOutput = calls.find((call) => {
        try {
          ...
          return (
            parsed.status === "error" &&
            parsed.message?.includes("Transcript file not found")
          );
        } catch {
          return false;
        }
  ...
  describe("error handling", () => {
    ...
    it("should handle archiver throwing exception", async () => {
      ...
      mockArchiver.archiveFromTranscript = jest
        .fn()
        .mockRejectedValue(new Error("Archiver crashed"));
