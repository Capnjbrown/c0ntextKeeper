...
const mockGetStoragePath = getStoragePath as jest.MockedFunction<
  typeof getStoragePath
>;
const mockGetHookStoragePath = getHookStoragePath as jest.MockedFunction<
  typeof getHookStoragePath
>;
...
describe("UserPromptSubmit Hook", () => {
  ...
  beforeEach(() => {
    ...
    mockWriteHookData.mockReturnValue(
      "/mock/storage/path/archive/projects/test-project/prompts/2024-01-01_1200_MT_sion-123-prompts.json",
    );
    mockGetHookStorageDir.mockReset();
    mockGetHookStorageDir.mockReturnValue(
      "/mock/storage/path/archive/projects/test-project/prompts",
    );
  ...
  describe("processUserPrompt", () => {
    const createValidInput = (
      overrides: Partial<UserPromptHookInput> = {},
    ): UserPromptHookInput => ({
    ...
    it("should detect code blocks in prompts", async () => {
      ...
      const storedData = mockWriteHookData.mock
        .calls[0][4] as UserPromptContext;
    ...
    it("should track follow-up prompts in same session", async () => {
      ...
      const firstInput = createValidInput({
        prompt: "First question about React",
      });
      ...
      const secondInput = createValidInput({
        prompt: "Follow up question about testing",
      });
    ...
    it("should not mark prompts from different sessions as follow-ups", async () => {
      ...
      const input = createValidInput({
        prompt: "New session question about API",
      });
      await processUserPrompt(input);
    ...
    it("should write single prompt context per file (per-session storage)", async () => {
      ...
      expect(storedData.prompt).toBe(
        "Help me fix this authentication bug in React",
      );
    });
    ...
    it("should filter sensitive data through SecurityFilter", async () => {
      const mockFilterText = jest.fn((text: string) =>
        text.replace(/secret/gi, "[REDACTED]"),
      );
      (SecurityFilter as jest.Mock).mockImplementation(() => ({
